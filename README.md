...
- 生徒管理機能
  - 生徒作成
    - 生徒の基本情報（名前、年齢、学年など）を登録
    - 科目、曜日、その科目に該当する講師を紐づける
  - 生徒の編集
    - 生徒の基本情報、科目、曜日、講師情報を編集
    - 今選択している科目を削除した場合、授業引継ぎが消える旨を表示
  - 生徒の削除
- 講師管理機能
  - 講師作成
    - 招待トークンを発行し、招待された講師のみが登録可能
  - 講師の編集
    - ここで曜日と科目を割り当てることができる
    - 指導している生徒がいる場合、直接編集できないようにする
  - 講師の削除
    - 生徒の指導担当から自動で外れる
- 生徒の特性管理機能
- 授業の引き継ぎ事項管理機能
...
### 生徒管理機能
#### 管理者ができること
- 生徒の作成・削除・編集のすべての操作が可能です。
- 生徒の名前をクリックすると、生徒ごとのダッシュボードに遷移します。
##### 権限
| 生徒作成ボタン                                           | 生徒の編集・削除ボタン                                      | 
| -------------------------------------------------------- | ----------------------------------------------------------- | 
| ![生徒の作成ボタン](./assets/gif/authorization/createStudentAdmin.gif) | ![生徒の編集・削除ボタン](./assets/gif/authorization/menuButtonAdmin.gif) | 
...
#### 講師ができること
- 生徒の削除・編集・削除はUI上に非表示にしています。
- 仮にURLを直接入力したとしても、権限エラーとなり操作できないようになっています。

##### ダッシュボード
![生徒ダッシュボード](./assets/gif/dashboard.gif)
...
- 積極的にユーザーフックを活用
### バックエンド
- サービス層、クエリ層、シリアライザ層の分離(Fat Model, Fat Controller回避)
- エラーハンドリングの共通化(Concern)
- カバレッジ80%以上の維持
