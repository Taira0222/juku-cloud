...
- ダッシュボードでは「生徒の特性」の一覧や、授業引継ぎを科目ごとに参照できます。
...
![引き継ぎ事項の作成](./assets/gif/lessonNote/createLessonNote.gif)

#### 引き継ぎ事項の編集
- 講師・管理者ともに、授業の引き継ぎ事項を編集できます。
- 更新をすると、最終更新者、最終更新日時が更新されます。

![引き継ぎ事項の編集](./assets/gif/lessonNote/editLessonNote.gif)

#### 引き継ぎ事項の削除
- 管理者のみが授業の引き継ぎ事項を削除できます。

![引き継ぎ事項の削除](./assets/gif/lessonNote/deleteLessonNote.gif)

### 生徒の特性管理機能
- 管理者のみが「特性管理」にアクセス出来て、作成・編集・削除が可能です。

#### 生徒の特性一覧
- 特性ごとにテーブルを選択することができます。

![生徒の特性一覧](./assets/gif/studentTrait/studentTraitList.gif)
...
| 講師登録機能                                  | ログイン機能                                  | ログアウト機能                                   | 
| --------------------------------------------- | --------------------------------------------- | ------------------------------------------------ | 
| ![講師登録機能](./assets/gif/auth/signUp.gif) | ![ログイン機能](./assets/gif/auth/signIn.gif) | ![ログアウト機能](./assets/gif/auth/signOut.gif) | 
...
#### 講師の削除
- 生徒の指導担当から自動で外れるようにする
### ダッシュボード機能
- 管理者・講師ともにアクセス可能。
- 「生徒の特性」の一覧や、「授業引継ぎ」を科目ごとに確認可能
### 授業の引き継ぎ事項管理機能
- 引き継ぎ事項は作成日時の降順で表示
#### 授業引継ぎの作成
- 授業の引継ぎの作成は科目ごとに行い、講師・管理者ともに可能
#### 授業引継ぎの編集
- 授業の引継ぎの編集は科目ごとに行い、講師・管理者ともに可能
#### 授業引継ぎの削除
- 授業の引継ぎの削除は管理者のみが可能
- 引き継ぎ事項は科目ごとに紐づいているため、科目を削除すると自動で引き継ぎ事項も削除される

### 生徒の特性管理機能
  - 講師は`dashboard`のみで生徒の特性の一覧を見ることが可能
  - 管理者のみが「特性管理」にアクセス出来て、作成・編集・削除が可能
#### 生徒の特性の作成
#### 生徒の特性の編集
#### 生徒の特性の削除
...
本番運用するために、最低限以下の機能追加・改善を予定しています。
